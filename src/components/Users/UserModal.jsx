...
const UserModal = ({ isOpen, onClose, user, onSuccess }) => {
  ...
  useEffect(() => {
    if (isOpen && isSuperAdmin && TENANT_REQUIRED_ROLES.includes(formData.role)) {
      fetchTenants();
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isSuperAdmin && TENANT_REQUIRED_ROLES.includes(formData.role) && !formData.tenantId) {
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-2xl max-h-[90vh] overflow-y-auto custom-scrollbar-dark">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs sm:text-sm text-white/80">First Name *</label>
              <input
                name="firstName"
            ...
            <div>
              <label className="text-xs sm:text-sm text-white/80">Last Name *</label>
              <input
                name="lastName"
          ...
          {/* Email */}
          <div>
            <label className="text-xs sm:text-sm text-white/80">Email *</label>
            <input
              type="email"
              name="email"
              ...
              className="input text-sm bg-white/5 border border-white/10 text-white placeholder-white/50"
            />
            {user && <p className="text-xs text-white/50 mt-1">Email cannot be changed</p>}
          </div>

          {/* Password */}
          <div>
            <label className="text-xs sm:text-sm text-white/80">Password {!user && '*'}</label>
            <input
              type="password"
              name="password"
          ...
          {/* Role */}
          <div>
            <label className="text-xs sm:text-sm text-white/80">Role *</label>
            <select
              name="role"
              value={formData.role}
              onChange={handleChange}
          ...
          {isSuperAdmin && TENANT_REQUIRED_ROLES.includes(formData.role) && (
            <div>
              <label className="text-xs sm:text-sm text-white/80">Tenant *</label>
              <select
                name="tenantId"
          ...
          {/* Status */}
          <div>
            <label className="text-xs sm:text-sm text-white/80">Status</label>
          ...
          <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 pt-4 border-t border-white/10">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-xs sm:text-sm text-white/70 hover:bg-white/10"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="rounded-xl bg-primary-500 px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-lg shadow-primary-500/20 hover:bg-primary-400"
            >
