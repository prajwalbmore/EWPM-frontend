...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const navItems = [
    ...
    { path: "/projects", icon: FolderKanban, label: "Projects", show: canDoTenantWork },
    { path: "/tasks", icon: CheckSquare, label: "Tasks", show: canDoTenantWork },
    { path: "/users", icon: Users, label: "Users", show: canViewUsers },
    { path: "/reports", icon: BarChart3, label: "Reports", show: canViewReports },
    { path: "/profile", icon: User, label: "Profile", show: true },
    { path: "/tenants", icon: Building2, label: "Tenants", show: canManageTenants },
    {
      path: "/permissions",
      icon: Shield,
      label: "Permissions",
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-50 h-full w-64 border-r border-white/10
        bg-slate-950/90 backdrop-blur-xl shadow-2xl
        transform transition-transform duration-300 ease-in-out
        ${isOpen ? "translate-x-0" : "-translate-x-full"}
        md:translate-x-0 md:top-16 md:z-30`}
      >
        {/* Mobile header */}
        <div className="flex items-center justify-between border-b border-white/10 p-4 md:hidden">
          ...
          <button
            onClick={onClose}
            className="rounded-lg p-2 text-white/70 hover:bg-white/10 hover:text-white transition active:scale-95"
        ...
        {/* Navigation */}
        <nav className="h-[calc(100vh-4rem)] overflow-y-auto p-3 sm:p-4 custom-scrollbar-dark">
              ...
              return (
                <li key={item.path}>
                  <NavLink
                    ...
                    onClick={onClose}
                    className={({ isActive }) =>
                      `group flex items-center gap-3 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base transition-all duration-200
                      ${
                        isActive
                          ? "bg-primary-500/15 text-primary-200 ring-1 ring-primary-500/30 shadow-inner"
